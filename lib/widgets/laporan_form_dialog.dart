...
class _LaporanFormDialogState extends State<_LaporanFormDialog> {
  ...
  // Kalau sedang mengubah laporan, kita cuma tahu NAMA kategorinya
  // (LaporanRow.kategori adalah String), bukan id-nya. Begitu daftar
  // kategori dari API selesai dimuat, id yang cocok dicari lewat nama ini.
  ...
  void initState() {
    ...
    // kategoriId akan dicocokkan dari nama setelah daftar kategori dari
    // API selesai dimuat (lihat _muatKategori).
    _pendingKategoriNama = e?.kategori;
    ...
    try {
      ...
      setState(() {
        ...
        if (_pendingKategoriNama != null) {
          // Mode Ubah: cocokkan nama kategori lama dengan daftar dari API.
          final match = data.where((k) => k.nama == _pendingKategoriNama);
          _kategoriId = match.isNotEmpty ? match.first.id : null;
        } else if (_kategoriId != null &&
            !data.any((k) => k.id == _kategoriId)) {
          // Kalau kategori lama sudah tidak ada di daftar (mis. sudah
          // dihapus admin), biarkan null supaya user pilih ulang.
          _kategoriId = null;
        }
      });
    } catch (e) {
